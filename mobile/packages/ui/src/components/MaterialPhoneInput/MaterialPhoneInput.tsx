...
export function MaterialPhoneInput({
	value,
	onChange,
	error,
	label = "Telemóvel",
		...
		<div className="relative">
			...
			<span
				className={cn(
					...
					error ? "text-error" : labelUp ? "text-primary" : "text-text-muted",
				)}
			>
				{label}
			</span>
...
function CountrySelect({
	...
	return (
		<div className="relative shrink-0">
			...
			>
				{Flag && (
					<span className="inline-flex w-5 h-4 shrink-0 overflow-hidden">
						<Flag title={country.label} />
					</span>
				)}
				<svg
					aria-hidden="true"
