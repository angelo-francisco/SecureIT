...
async def delete_role(role_id: int, user_id: int) -> None:
    role = await get_role(role_id, user_id)
    in_use = await PersonRole.filter(role_id=role_id).count()
    if in_use > 0:
        raise ValidationError_(
            f"Não é possível eliminar este cargo: está a ser utilizado por {in_use} pessoa(s). "
            "Remova o cargo das pessoas primeiro."
        )
    await role.delete()
