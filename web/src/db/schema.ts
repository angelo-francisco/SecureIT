...
export const licenseKey = sqliteTable(
  ...
  (t) => [index("LicenseKey_key_idx").on(t.key), uniqueIndex("LicenseKey_key_key").on(t.key)],
);
...
export const license = sqliteTable(
  "License",
  {
    ...
    keyId: text("keyId")
      .notNull()
      .references(() => licenseKey.id, { onDelete: "restrict", onUpdate: "cascade" }),
    userId: text("userId")
      .notNull()
      .references(() => user.id, { onDelete: "restrict", onUpdate: "cascade" }),
    paymentRequestId: text("paymentRequestId").references(() => paymentRequest.id, {
      onDelete: "set null",
      onUpdate: "cascade",
    }),
...
export const paymentRequest = sqliteTable(
  "PaymentRequest",
  {
    ...
    paymentInfoId: text("paymentInfoId")
      .notNull()
      .references(() => paymentInfo.id, { onDelete: "restrict", onUpdate: "cascade" }),
...
export const maintenanceRequest = sqliteTable("MaintenanceRequest", {
  ...
  adminNote: text("adminNote"),
  hasPaidLicense: integer("hasPaidLicense", { mode: "boolean" }).notNull().default(false),
  totalPrice: real("totalPrice"),
  createdAt: text("createdAt")
    .notNull()
    .$defaultFn(() => new Date().toISOString()),
