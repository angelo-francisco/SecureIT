...
describe("POST /api/admin/licenses/generate", () => {
	it("generate B2C x2 returns count=2", async () => {
		const { POST: generatePost } = await import(
			"@/app/api/admin/licenses/generate/route"
		);
		const req = makeAdminRequest(
			"http://localhost/api/admin/licenses/generate",
			{
				method: "POST",
				body: { type: "B2C", durationDays: 30, quantity: 2 },
			},
		);
	...
	it("generate B2B x1 returns type=B2B", async () => {
		...
		const req = makeAdminRequest(
			...
			{
				method: "POST",
				body: { type: "B2B", durationDays: 7, quantity: 1 },
			},
		);
		...
		expect(data.licenses[0].type).toBe("B2B");
		expect(data.licenses[0].maxCameras).toBe(-1);
		expect(data.licenses[0].maxPeople).toBe(-1);
...
describe("GET /api/admin/licenses", () => {
	it("returns list with pagination", async () => {
		...
		const req = makeAdminRequest(
			"http://localhost/api/admin/licenses?page=1&limit=10",
		);
...
describe("GET /api/admin/licenses/[id]", () => {
	it("returns key details", async () => {
		const { GET: licenseGet } = await import(
			"@/app/api/admin/licenses/[id]/route"
		);
		...
		const req = makeAdminRequest(
			`http://localhost/api/admin/licenses/${targetId}`,
		);
		const response = await licenseGet(req, {
	...
	it("nonexistent returns 404", async () => {
		...
		const req = makeAdminRequest(
			"http://localhost/api/admin/licenses/nonexistent-id",
		);
		const response = await licenseGet(req, {
...
describe("DELETE /api/admin/licenses/[id]", () => {
	it("revokes license (sets status=REVOKED)", async () => {
		const { DELETE: licenseDelete } = await import(
			"@/app/api/admin/licenses/[id]/route"
		);
		...
		try {
			const req = makeAdminRequest(
				`http://localhost/api/admin/licenses/${createdId}`,
				{
					method: "DELETE",
				},
			);
			const response = await licenseDelete(req, {
...
describe("unauthenticated admin access", () => {
	it("returns 401 when not authenticated", async () => {
		...
		try {
			const { GET: licensesGet } = await import(
				"@/app/api/admin/licenses/route"
			);
