      ...
      <Modal open={plansModalOpen} disableBackdropClose className="w-full max-w-5xl mx-4">
        <div className="bg-surface border border-border p-6 max-h-[85vh] overflow-y-auto">
          <div className="flex items-center justify-center gap-2.5 mb-5">
            <h2 className="text-xl md:text-2xl font-display font-bold text-text">Planos e Pagamentos</h2>
          </div>
          <PlansSection ref={plansSectionRef} data={plansData} onClose={() => setPlansModalOpen(false)} />
        </div>
      </Modal>
        ...
        <div className="relative" ref={menuRef}>
          ...
          {menuOpen && (
            <div className="absolute bottom-full right-0 mb-2 bg-surface border border-border shadow-lg min-w-[250px] divide-y divide-gray-700 z-50 animate-slide-up">
              <button
                onClick={() => { setMenuOpen(false); openPlansModal(); }}
              ...
              <button
                onClick={async () => { setMenuOpen(false); await fetchPaymentInfo(); setMaintenanceModalOpen(true); }}
                className="w-full flex items-center gap-3 px-4 py-3 text-left text-base md:text-lg font-medium text-text hover:bg-surface-hover transition-all"
              >
              ...
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-3 text-left text-base md:text-lg font-medium text-text hover:bg-surface-hover transition-all"
              >
                {loggingOut ? <Loader w={16} /> : <LogOut size={18} className="text-red-500" />}
