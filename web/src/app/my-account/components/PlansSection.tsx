								...
								>
									<div className="absolute bottom-5 right-5">
										{selectedPlan?.id === plan.id ? (
											...
											<div className="w-5 h-5 rounded-full bg-transparent border flex items-center justify-center"></div>
										)}
									</div>
									<h3 className="text-xl md:text-2xl font-bold text-text">
										{plan.name}
									</h3>
									<p className="text-base md:text-lg text-text-muted mb-3">
										{plan.description}
									</p>
									...
									<div className="mt-3 space-y-1">
										{INCLUDED_FEATURES.map((f) => (
											<div
												key={f}
												className="flex items-center gap-1.5 text-lg text-text-muted"
											>
				...
				{step === 2 && selectedPlan && (
					<>
						{selectedPlan.services.length > 0 &&
							selectedPlan.services.some((s) => s.price > 0) && (
								<div>
									<h4 className="text-base md:text-xl text-text">
										Serviços e Funcionalidades (Opcionais)
									</h4>
										...
										{selectedPlan.services
											...
											.map((service) => {
												...
												return (
													<label
														key={service.id}
														className="flex items-center px-4 justify-between bg-surface cursor-pointer hover:border-primary/50 transition-all"
													>
                            ...
                            after:opacity-0 checked:after:opacity-100"
															...
															<div className="flex gap-1">
																<span className="text-base md:text-xl font-semibold text-tex">
																	{service.name}
																</span>
																{service.description && (
																	<div
								...
								<div className="border bg-surface p-4 space-y-2">
									...
									<div className="flex items-center justify-between">
										...
										<span className="text-lg text-primary">
											{convert(Number(total.toFixed(2)))} Kz
										</span>
									</div>
								</div>
							) : (
						...
						<div className="bg-surface border border-border p-4">
							<div className="flex items-center justify-between">
								...
								<span className="text-lg md:text-xl text-text">
							...
							{selectedPlan.services
								.filter((s) => selectedServices.includes(s.id) && s.price > 0)
								.map((s) => (
									<div
										key={s.id}
							...
							<div className="border-t border-border mt-2 pt-2 flex items-center justify-between">
								<span className="text-lg md:text-xl font-semibold text-text">
									Total
								</span>
